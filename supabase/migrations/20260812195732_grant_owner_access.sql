-- Every auth user needs a profile row before they can be a member.
-- This trigger creates it automatically from now on.
...
-- Make the account holder owner of The Greek Club.
insert into property_members (property_id, user_id, role)
select p.id, u.id, 'owner'
from properties p, auth.users u
where p.slug = 'greek-club-dahab'
  and u.email = 'moataz4533@gmail.com'
