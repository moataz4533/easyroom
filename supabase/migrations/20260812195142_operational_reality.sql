alter table room_allocations add column release_reason text;
...
declare
  v_total numeric := 0; v_heads int := 0; v_plan uuid; v_prop uuid; a record;
begin
...
-- Ask before doing, so reception can answer the guest on the phone.
create or replace function check_extension(p_booking uuid, p_new_check_out date)
returns table (
  allocation_id uuid, room_id uuid, room_number text,
  can_extend boolean, blocked_from date, blocked_by text
)
language sql stable security invoker set search_path = public as $$
  select a.id, a.room_id, r.number,
    conflict.id is null,
    conflict.starts_on,
    case
...
create or replace function suggest_alternative_rooms(
  p_allocation uuid, p_from date default null, p_to date default null
)
returns table (room_id uuid, room_number text, type_name text, same_type boolean)
language sql stable security invoker set search_path = public as $$
  with a as (
  ...
  where r.is_active
    ...
    and not exists (
      ...
      where c.room_id = r.id and c.released_at is null
...
-- A mid-stay move splits the allocation so each room bills its own nights.
create or replace function move_room(
  p_allocation uuid, p_new_room uuid, p_from date default null, p_reason text default null
)
returns room_allocations
language plpgsql volatile security invoker set search_path = public as $$
declare
  v_alloc room_allocations; v_new room_allocations; v_split date; v_max int;
begin
  ...
  if v_split <= v_alloc.starts_on then
    ...
    if v_split >= v_alloc.ends_on then raise exception 'move date is after the stay ends'; end if;
...
-- A broken room is a fact, not a request: the block always wins.
-- Affected bookings are released, flagged, AND returned so the interface
-- can offer relocations instead of quietly losing a reservation.
create or replace function block_room(
  p_room uuid, p_from date, p_to date,
  p_reason text default 'صيانة', p_kind allocation_kind default 'maintenance'
)
returns table (
  booking_id uuid, reference text, guest_name text,
  guest_phone text, starts_on date, ends_on date, occupancy int
)
language plpgsql volatile security invoker set search_path = public as $$
declare v_prop uuid;
begin
  ...
  if not can_manage(v_prop) then raise exception 'not authorised' using errcode = '42501'; end if;
  ...
  where a.room_id = p_room and a.released_at is null
    and a.stay && daterange(p_from, p_to, '[)');

  update room_allocations set released_at = now(), release_reason = p_reason
   where id in (select alloc_id from _displaced);

  update bookings set attention_reason = format('الغرفة اتعطلت: %s', p_reason)
  ...
  return query
    select d.booking_id, d.reference, d.full_name, d.phone, d.starts_on, d.ends_on, d.occupancy
...
begin
  ...
  update room_allocations set released_at = now(), release_reason = 'انتهت الصيانة'
...
-- The one list a manager checks. Empty means nothing is stranded.
create or replace view attention_queue as
select
  b.property_id, b.id as booking_id, b.reference, b.status,
  b.check_in, b.check_out, g.full_name as guest_name, g.phone as guest_phone,
  coalesce(
    b.attention_reason,
    case when not exists (
      select 1 from room_allocations a where a.booking_id = b.id and a.released_at is null
...
select a.property_id, r.id as room_id, r.number,
       a.starts_on, a.ends_on, a.kind, a.notes as reason
from room_allocations a
join rooms r on r.id = a.room_id
where a.kind in ('maintenance', 'hold', 'staff')
  and a.released_at is null and a.ends_on > current_date;
