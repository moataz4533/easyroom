...
/**
 * Whether the hotel about to be emptied is the hotel that was named.
 *
 * The security on a reset is `is_platform_admin()` in the database, and it
 * is not this. This is for the hand: the console lists hotels one under
 * another, and the price of pressing the wrong row must not be a hotel. So
 * the code is typed back before the button will do anything, and it is
 * checked here as well as in the database — here so the operator is told
 * before pressing, there because a check that only exists in a screen is
 * not a check.
 */
export function resetProblem(typed, slug) {
  if (!String(typed || "").trim()) return "needCode";
  if (normaliseCode(typed) !== normaliseCode(slug)) return "codeMismatch";
  return null;
}
