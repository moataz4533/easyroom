 ...
 */
import { hotelDialCode } from "./hotel-settings";

/**
 * A phone number as two people would have to type it to mean the same one.
 *
 * A number is written both ways everywhere: +201118070453 and 01118070453
 * are one phone and must match each other. Which prefix means "this
 * country" was Egypt's, written in — so for a Saudi hotel `+9665…` and
 * `05…` never matched, and one guest would quietly get two records. That is
 * the same fault that put twelve rows on one number here, arriving by a
 * different door.
 *
 * The code comes from the hotel; passing one explicitly is for tests.
 */
export function normalisePhone(value, dialCode = null) {
  const digits = String(value || "").replace(/[^\d+]/g, "");
  if (!digits) return "";
  const code = String(dialCode || hotelDialCode()).replace(/\D/g, "");
  if (!code) return digits;
  if (digits.startsWith(`+${code}`)) return `0${digits.slice(code.length + 1)}`;
  if (digits.startsWith(`00${code}`)) return `0${digits.slice(code.length + 2)}`;
  if (digits.startsWith(code) && digits.length > 10) return `0${digits.slice(code.length)}`;
  return digits;
}
