 ...
 * { pricedRates, types, rooms, hasPin, staffCount, settings }
 */
export function outstandingSetup(state) {
  ...
  /**
   * Priced rows, not rows. This counted rate rows and passed on eight of
   * them — every one of which was 0.00, because the matrix is created
   * empty and a row saved at zero is still a row. So the hotel was told
   * its prices were done while every booking it took priced at nothing,
   * which is the one mistake this checklist exists to prevent.
   */
  if (!state?.pricedRates) {
