...
const HEADERS = {
  ar: {
    ...
    extras: "إيراد الإضافات", discounts: "الخصومات",
  },
  en: {
    ...
    extras: "Extras revenue", discounts: "Discounts given",
  },
};
...
export function buildRevenueWorkbook({
  ...
  // What was given away. Below the totals rather than in a column, because
  // the revenue above it is already net of it — this says how much room
  // there was between the rate card and the till.
  const discounts = num(summary?.discounts);
  if (discounts > 0) {
    if (extras <= 0) body.push([]);
    body.push([
      { text: t.discounts, style: STYLE.total }, "", "", "", "",
      { number: Number(discounts.toFixed(2)), style: STYLE.money },
    ]);
  }
