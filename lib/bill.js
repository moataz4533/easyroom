...
const WORDS = {
  ...
  ru: {
    title: "Счёт", booking: "Бронь", guest: "Гость",
    stay: "Проживание", rooms: "Номера", extras: "Дополнительно", payments: "Платежи",
    total: "Итого", paid: "Оплачено", balance: "К оплате", settled: "Оплачено полностью",
    nights: "{n} ноч.", pax: "{n} гост.", currency: "EGP",
    discount: "Скидка", saved: "Вы сэкономили", percent: "%", thanks: "Благодарим за проживание у нас.",
  },
  de: {
    title: "Rechnung", booking: "Buchung", guest: "Gast",
    stay: "Aufenthalt", rooms: "Zimmer", extras: "Zusatzleistungen", payments: "Zahlungen",
    total: "Gesamt", paid: "Bezahlt", balance: "Offener Betrag", settled: "Vollständig bezahlt",
    nights: "{n} Nächte", pax: "{n} Gäste", currency: "EGP",
    discount: "Rabatt", saved: "Sie sparen", percent: "%", thanks: "Danke für Ihren Aufenthalt bei uns.",
  },
  it: {
    title: "Conto", booking: "Prenotazione", guest: "Ospite",
    stay: "Soggiorno", rooms: "Camere", extras: "Extra", payments: "Pagamenti",
    total: "Totale", paid: "Pagato", balance: "Saldo", settled: "Saldato",
    nights: "{n} notti", pax: "{n} ospiti", currency: "EGP",
    discount: "Sconto", saved: "Hai risparmiato", percent: "%", thanks: "Grazie per aver soggiornato da noi.",
  },
};
...
export function buildBillText({ property, booking, allocations, charges, payments, locale = "ar" }) {
  const w = labelsFor(WORDS, locale);
  const n = (value) => formatNumber(value, locale);
  // The hotel's currency, not the one frozen into the label table.
  const cash = (value) => `${n(money(value))} ${currencyWord(locale)}`;
  const totals = billTotals({ allocations, charges, payments });
  const out = [];

  const hotel = (locale === "ar" ? null : property?.name_en) || property?.name || "";
  out.push(`*${w.title}*`);
