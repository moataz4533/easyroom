 ...
 */
export const BILL_ALLOCATION_FIELDS = ["starts_on", "ends_on", "rate_per_night", "release_reason"];
...
export function billTotals({ allocations, charges, payments }) {
  const rooms = roomLines(allocations).reduce((sum, l) => sum + l.total, 0);
...
export function buildBillText({ property, booking, allocations, charges, payments, locale = "ar" }) {
  ...
  if (rooms.length) {
    ...
    for (const line of rooms) {
      out.push(`${line.label}`);
      out.push(`  ${w.nights.replace("{n}", n(line.nights))} × ${cash(line.rate)} = ${cash(line.total)}`);
    }
    out.push("");
  }
