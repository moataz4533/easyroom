...
import { clockLabel, hotelHours } from "./hotel-hours";
...
const WORDS = {
  en: {
    ...
    checkOut: "Departure",
    hours: "Hotel hours",
    hoursValue: "Check-in from {in} · Check-out by {out}",
    nights: "Nights",
    party: "Guests",
    rooms: "Accommodation",
  ...
  ar: {
    ...
    hours: "مواعيد الفندق",
    hoursValue: "الوصول من {in} · المغادرة حتى {out}",
    nights: "عدد الليالي",
    party: "عدد الأفراد",
    rooms: "الإقامة",
...
export const PASS_SECTIONS = [
  ...
  { key: "staySection", fields: ["checkIn", "checkOut", "nights", "party", "rooms", "hours"] },
];
...
function hotelHead(property) {
  ...
  const hours = hotelHours(property);
  return {
    ...
    // `whatsapp_number` is the key the settings screen writes. This read
    // `settings.whatsapp`, so a hotel that had filled the box in printed a
    // paper with no number on it and no way to tell why.
    phone: settings.phone || settings.whatsapp_number || settings.whatsapp || "",
...
/** "Check-in from 2:00 PM · Check-out by 12:00 PM", the way a voucher says it. */
export function hoursLine(model, locale = "en") {
  const w = passWords(locale);
  const from = clockLabel(model.hotel.checkInTime, locale);
  const until = clockLabel(model.hotel.checkOutTime, locale);
  if (!from || !until) return "";
  return w.hoursValue.replace("{in}", from).replace("{out}", until);
}
