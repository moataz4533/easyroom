import { hotelCurrency } from "./hotel-settings";
// One tag per language a guest's paperwork can be written in. Missing tags
// fell back to Arabic, which would have printed ٢٠٢٦ and ١٤٠٠ inside a
// Russian confirmation — numbers the guest cannot read at all.
export const LOCALE_TAGS = {
  ar: "ar-EG", en: "en-GB", ru: "ru-RU", de: "de-DE", it: "it-IT",
};
...
export function formatCurrency(value, locale = "ar") {
  return new Intl.NumberFormat(LOCALE_TAGS[locale] || LOCALE_TAGS.ar, {
    style: "currency", currency: hotelCurrency(), currencyDisplay: "narrowSymbol", maximumFractionDigits: 0,
...
/**
 * A date written out in full, weekday included.
 *
 * Reception said the date fields "don't work properly". A native date input
 * shows whatever the phone's locale says — often year first, in Arabic-Indic
 * digits — which is easy to read as the day. So whatever the widget shows,
 * the screen says the chosen day back in words underneath it. The weekday is
 * the part that catches a wrong pick: nobody books a Thursday and reads
 * "Sunday" without noticing.
 */
export function fullDate(iso, locale = "ar") {
